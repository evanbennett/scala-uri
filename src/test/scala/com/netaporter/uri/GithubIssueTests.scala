...
class GithubIssueTests extends TestSpec {
  ...
  "Github Issue #2" should "now be fixed. Pluses in querystrings should be encoded when using ``UriConfig.CONSERVATIVE`" in {
    val uri: Uri = "http".`://`("theon.github.com") /? ("+" `=` "+") // THEON: Updated to new DSL.
  ...
  "Github Issue #4" should "now be fixed. Port numbers should be rendered by toString" in {
    val uri: Uri = "http".`://`("theon.github.com" `:` 8080) / "test" ? ("p" `=` "1") // THEON: Updated to new DSL.
  ...
  "Github Issue #6" should "now be fixed. No implicit Encoder val required for implicit Uri -> String conversion " in {
    val uri: Uri = /("blah") ? ("blah" `=` "blah") // THEON: Updated to new DSL.
  ...
  "Github Issue #8" should "now be fixed. Parsed relative uris should have no scheme" in {
    val uri = Uri("abc") // THEON: Updated to new `Uri` parsing method.
    uri.scheme should equal(None)
    uri.hostString should equal(None) // THEON: Updated to new host string method.
  ...
  "Github Issue #12" should "now be fixed. Parsing URIs parse percent escapes" in {
    val source = Uri(
      ...
      Query.option(Seq(Parameter("a b", Some("c d")))),
      None
    )
    val parsed = Uri(source.toString) // THEON: Updated to new `Uri` parsing method.
    parsed should equal(source)
  }

  "Github Issue #15" should "now be fixed. Empty Query String values are parsed" in {
    val uri = Uri("http://localhost:8080/ping?oi=TscV16GUGtlU&ppc=&bpc=") // THEON: Updated to new `Uri` parsing method.
    uri.scheme.value.scheme should equal("http")
    uri.hostString.value should equal("localhost") // THEON: Updated to new host string method.
    uri.port.value should equal(8080)
    uri.pathToString should equal("/ping")
    uri.queryParameters.value should equal(Seq( // THEON: Updated to new `Query` structure.
  ...
  "Github Issue #19" should "now be fixed" in {
    val uri: Uri = /("coldplay.com") ? ("singer" `=` "chris&will") // THEON: Updated to new DSL.
    uri.toString should equal("/coldplay.com?singer=chris%26will")
  }

  "Github Issue #26" should "now be fixed" in {
    implicit val config = UriConfig.DEFAULT.withDecoding(PermissivePercentDecoder) // THEON: Changed to new builder.
    val uri = Uri("http://lesswrong.com/index.php?query=abc%yum&john=hello") // THEON: Updated to new `Uri` parsing method.
    uri.queryParameters.value should equal(Seq( // THEON: Updated to new `Query` structure.
  ...
  "Github Issue #56" should "now be fixed" in {
    val ex = the[java.net.URISyntaxException] thrownBy Uri("http://test.net/##")(UriConfig.RFC3986) // THEON: Updated to new `Uri` parsing method and made to fail.
    ex.getMessage should startWith("Invalid URI could not be parsed.")
  }

  "Github Issue #65 example 1" should "now be fixed" in {
    val uri = Uri("http://localhost:9000/?foo=test&&bar=test") // THEON: Updated to new `Uri` parsing method.
    uri.toString should equal("http://localhost:9000/?foo=test&bar=test") // THEON: Removed the double '&' due to functional changes.
  }

  "Github Issue #65 example 2" should "now be fixed" in {
    val uri = Uri("http://localhost:9000/mlb/2014/06/15/david-wrights-slump-continues-why-new-york-mets-franchise-third-baseman-must-be-gone-before-seasons-end/?utm_source=RantSports&utm_medium=HUBRecirculation&utm_term=MLBNew York MetsGrid")(UriConfig.DEFAULT.copy(delimiterParsing = true)) // THEON: Updated to new `Uri` parsing method.
  ...
  "Github Issue #65 example 3" should "now be fixed" in {
    val uri = Uri("http://localhost:9000/t?x=y%26") // THEON: Updated to new `Uri` parsing method.
    uri.queryParameters.value should equal(Seq(Parameter("x", Some("y&")))) // THEON: Updated to new `Query` structure.
    uri.toString should equal("http://localhost:9000/t?x=y%26")
  }

  "Github Issue #65 example 4" should "now be fixed" in {
    val uri = Uri("http://localhost/offers.xml?&id=10748337&np=1") // THEON: Updated to new `Uri` parsing method.
    uri.toString should equal("http://localhost/offers.xml?id=10748337&np=1") // THEON: Removed the initial '&' due to functional changes.
  }

  "Github Issue #65 example 5" should "now be fixed" in {
    val uri = Uri("http://localhost/offers.xml?id=10748337&np=1&") // THEON: Updated to new `Uri` parsing method.
    uri.toString should equal("http://localhost/offers.xml?id=10748337&np=1") // THEON: Removed the last '&' due to functional changes.
  }

  "Github Issue #65 example 6" should "now be fixed" in {
    val uri = Uri("http://localhost/offers.xml?id=10748337&np=1&#anchor") // THEON: Updated to new `Uri` parsing method.
    uri.toString should equal("http://localhost/offers.xml?id=10748337&np=1#anchor") // THEON: Removed the last '&' due to functional changes.
  }

  "Github Issue #68" should "now be fixed" in {
    val uri: Uri = "http".`://`("example.com") / "path" ? ("param" `=` "something==") // THEON: Updated to new DSL.
    uri.toString should equal("http://example.com/path?param=something%3D%3D")
  }

  "Github Issue #72" should "now be fixed" in {
    val uri = Uri("http://hello.world?email=abc@xyz") // THEON: Updated to new `Uri` parsing method.
    uri.hostString.value should equal("hello.world") // THEON: Updated to new host string method.
    uri.queryParameters.value should equal(Seq(Parameter("email", Some("abc@xyz")))) // THEON: Updated to new `Query` structure.
  }

  "Github Issue #73" should "now be fixed" in {
    val uri = Uri("http://somewhere.something").withUser("user:1@domain").withPassword("abc xyz") // THEON: Converted to direct parsing. // THEON: Updated to new `Uri` parsing method.
    uri.toString should equal("http://user%3A1%40domain:abc%20xyz@somewhere.something")
  }

  "Github Issue #85" should "now be fixed" in {
    val uri = Uri("http://localhost:8080?VkSoj=2Xobp@") // THEON: Updated to new `Uri` parsing method.
    uri.scheme.value.scheme should equal("http")
    uri.hostString.value should equal("localhost") // THEON: Updated to new host string method.
    uri.port.value should equal(8080)
    uri.path should equal(None)
    uri.queryParameters.value should equal(Seq(Parameter("VkSoj", Some("2Xobp@")))) // THEON: Updated to new `Query` structure.
    uri.fragment should equal(None)
  }

  "Github Issue #99" should "now be fixed" in {
    val uri = Uri("https://www.foo.com/#/myPage?token=bar") // THEON: Updated to new `Uri` parsing method.
    uri.toString should equal("https://www.foo.com/#/myPage?token=bar")
  }

  "Github Issue #104" should "now be fixed" in {
    val uri = Uri("a1+-.://localhost") // THEON: Updated to new `Uri` parsing method.
    uri.scheme.value.scheme should equal("a1+-.")
    uri.hostString.value should equal("localhost") // THEON: Updated to new host string method.
  }

  "Github Issue #106" should "now be fixed" in {
    val p = "http".`://`("localhost" `:` 1234) // THEON: Updated to new DSL.
    val withPath: Uri = p / "some" / "path" / "segments" // THEON: Converted to individual segments due to functional changes (the '/'s within a segment get encoded).
    withPath.toString should equal("http://localhost:1234/some/path/segments")
    val withPathAndQuery: Uri = p / "some" / "path" / "segments" ? ("returnUri" `=` "http://localhost:1234/some/path/segments") // THEON: Updated to new DSL.
    withPathAndQuery.toString should equal("http://localhost:1234/some/path/segments?returnUri=http://localhost:1234/some/path/segments")
  }
