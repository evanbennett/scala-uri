...
object UpdatePublicSuffixTrie {
  ...
  case class Trie(children: Map[Char, Trie], wordEnd: Boolean = false) {
    ...
    def +(kv: (Char, Trie)): Trie = this.copy(children = children + kv)

    def next(ch: Char): Option[Trie] = children.get(ch)

    def insert(value: List[Char]): Trie = value match {
      case Nil => copy(wordEnd = true)
      case x :: xs => next(x) match {
        case None => this + (x -> Trie(xs))
        case Some(child) => this + (x -> child.insert(xs))
      }
    }
